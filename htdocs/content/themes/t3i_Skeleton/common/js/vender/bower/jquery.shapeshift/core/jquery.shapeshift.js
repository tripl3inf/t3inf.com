  ...
  (function(jQuery, window, document) {
    ...
    Plugin = (function() {
      ...
      function Plugin(element, options) {
        ...
        this.globals = {};
        this.jQuerycontainer = jQuery(element);
        if (this.errorCheck()) {
          this.init();
        }
      }
      ...
      Plugin.prototype.errorCheck = function() {
        var jQuerychildren, error_msg, errors, options;
        options = this.options;
        errors = false;
        error_msg = "Shapeshift ERROR:";
        if (options.colWidth === null) {
          jQuerychildren = this.jQuerycontainer.children(options.selector);
          if (jQuerychildren.length === 0) {
            errors = true;
      ...
      Plugin.prototype.createEvents = function() {
        var jQuerycontainer, options,
          _this = this;
        options = this.options;
        jQuerycontainer = this.jQuerycontainer;
        jQuerycontainer.off("ss-arrange").on("ss-arrange", function(e, trigger_drop_finished) {
          if (trigger_drop_finished == null) {
            trigger_drop_finished = false;
          }
          return _this.render(false, trigger_drop_finished);
        });
        jQuerycontainer.off("ss-rearrange").on("ss-rearrange", function() {
          return _this.render(true);
        });
        jQuerycontainer.off("ss-setTargetPosition").on("ss-setTargetPosition", function() {
          return _this.setTargetPosition();
        });
        return jQuerycontainer.off("ss-destroy").on("ss-destroy", function() {
      ...
      Plugin.prototype.setIdentifier = function() {
        ...
        return this.jQuerycontainer.addClass(this.identifier);
      };
      ...
      Plugin.prototype.setActiveChildren = function() {
        var jQuerychildren, active_child_class, colspan, columns, i, min_columns, options, total, _i, _j, _ref, _results;
        options = this.options;
        jQuerychildren = this.jQuerycontainer.children(options.selector);
        active_child_class = options.activeClass;
        total = jQuerychildren.length;
        for (i = _i = 0; 0 <= total ? _i < total : _i > total; i = 0 <= total ? ++_i : --_i) {
          jQuery(jQuerychildren[i]).addClass(active_child_class);
        }
      ...
      Plugin.prototype.setParsedChildren = function() {
        var jQuerychild, jQuerychildren, child, i, parsedChildren, total, _i;
        jQuerychildren = this.jQuerycontainer.find("." + this.options.activeClass).filter(":visible");
        total = jQuerychildren.length;
        parsedChildren = [];
        for (i = _i = 0; 0 <= total ? _i < total : _i > total; i = 0 <= total ? ++_i : --_i) {
          jQuerychild = jQuery(jQuerychildren[i]);
          child = {
            i: i,
            el: jQuerychild,
            colspan: parseInt(jQuerychild.attr("data-ss-colspan")) || 1,
            height: jQuerychild.outerHeight()
          };
      ...
      Plugin.prototype.setGridColumns = function() {
        ...
        inner_width = this.jQuerycontainer.innerWidth() - (padding_x * 2);
      ...
      Plugin.prototype.arrange = function(reparse, trigger_drop_finished) {
        var jQuerychild, jQuerycontainer, animated, animation_speed, attributes, child_positions, container_height, dragged_class, globals, i, is_dragged_child, max_height, min_height, options, parsed_children, placeholder_class, total_children, _i;
        if (reparse) {
          this.setParsedChildren();
        }
        globals = this.globals;
        ...
        for (i = _i = 0; 0 <= total_children ? _i < total_children : _i > total_children; i = 0 <= total_children ? ++_i : --_i) {
          jQuerychild = parsed_children[i].el;
          attributes = child_positions[i];
          is_dragged_child = jQuerychild.hasClass(dragged_class);
          if (is_dragged_child) {
            placeholder_class = options.placeholderClass;
            jQuerychild = jQuerychild.siblings("." + placeholder_class);
          }
          if (animated && !is_dragged_child) {
            jQuerychild.stop(true, false).animate(attributes, animation_speed, function() {});
          } else {
            jQuerychild.css(attributes);
          }
        }
        if (trigger_drop_finished) {
          if (animated) {
            setTimeout((function() {
              return jQuerycontainer.trigger("ss-drop-complete");
            }), animation_speed);
          } else {
            jQuerycontainer.trigger("ss-drop-complete");
          }
        }
        jQuerycontainer.trigger("ss-arranged");
        if (options.autoHeight) {
          ...
          return jQuerycontainer.height(container_height);
        }
      };
      ...
      Plugin.prototype.enableDragNDrop = function() {
        var jQueryclone, jQuerycontainer, jQueryplaceholder, jQueryselected, active_class, clone_class, current_container_class, delete_clone, drag_clone, drag_rate, dragged_class, dragging, options, original_container_class, placeholder_class, previous_container_class, selected_offset_x, selected_offset_y,
          _this = this;
        options = this.options;
        jQuerycontainer = this.jQuerycontainer;
        ...
        jQueryselected = jQueryplaceholder = jQueryclone = selected_offset_y = selected_offset_x = null;
        dragging = false;
        if (options.enableDrag) {
          jQuerycontainer.children("." + active_class).filter(options.dragWhitelist).draggable({
            addClasses: false,
            ...
            start: function(e, ui) {
              var selected_tag;
              jQueryselected = jQuery(e.target);
              if (drag_clone) {
                jQueryclone = jQueryselected.clone(true).insertBefore(jQueryselected).addClass(clone_class);
              }
              jQueryselected.addClass(dragged_class);
              selected_tag = jQueryselected.prop("tagName");
              jQueryplaceholder = jQuery("<" + selected_tag + " class='" + placeholder_class + "' style='height: " + (jQueryselected.height()) + "px; width: " + (jQueryselected.width()) + "'></" + selected_tag + ">");
              jQueryselected.parent().addClass(original_container_class).addClass(current_container_class);
              selected_offset_y = jQueryselected.outerHeight() / 2;
              return selected_offset_x = jQueryselected.outerWidth() / 2;
            },
            drag: function(e, ui) {
              if (!dragging && !(drag_clone && delete_clone && jQuery("." + current_container_class)[0] === jQuery("." + original_container_class)[0])) {
                jQueryplaceholder.remove().appendTo("." + current_container_class);
                jQuery("." + current_container_class).trigger("ss-setTargetPosition");
                dragging = true;
                window.setTimeout((function() {
                  return dragging = false;
                }), drag_rate);
              }
              ui.position.left = e.pageX - jQueryselected.parent().offset().left - selected_offset_x;
              return ui.position.top = e.pageY - jQueryselected.parent().offset().top - selected_offset_y;
            },
            stop: function() {
              var jQuerycurrent_container, jQueryoriginal_container, jQueryprevious_container;
              jQueryoriginal_container = jQuery("." + original_container_class);
              jQuerycurrent_container = jQuery("." + current_container_class);
              jQueryprevious_container = jQuery("." + previous_container_class);
              jQueryselected.removeClass(dragged_class);
              jQuery("." + placeholder_class).remove();
              if (drag_clone) {
                if (delete_clone && jQuery("." + current_container_class)[0] === jQuery("." + original_container_class)[0]) {
                  jQueryclone.remove();
                  jQuery("." + current_container_class).trigger("ss-rearrange");
                } else {
                  jQueryclone.removeClass(clone_class);
                }
              }
              if (jQueryoriginal_container[0] === jQuerycurrent_container[0]) {
                jQuerycurrent_container.trigger("ss-rearranged", jQueryselected);
              } else {
                jQueryoriginal_container.trigger("ss-removed", jQueryselected);
                jQuerycurrent_container.trigger("ss-added", jQueryselected);
              }
              jQueryoriginal_container.trigger("ss-arrange").removeClass(original_container_class);
              jQuerycurrent_container.trigger("ss-arrange", true).removeClass(current_container_class);
              jQueryprevious_container.trigger("ss-arrange").removeClass(previous_container_class);
              return jQueryselected = jQueryplaceholder = null;
            }
          });
        }
        if (options.enableCrossDrop) {
          return jQuerycontainer.droppable({
            accept: options.crossDropWhitelist,
            tolerance: 'intersect',
            over: function(e) {
              jQuery("." + previous_container_class).removeClass(previous_container_class);
              jQuery("." + current_container_class).removeClass(current_container_class).addClass(previous_container_class);
              return jQuery(e.target).addClass(current_container_class);
            },
            drop: function(e, selected) {
              ...
              if (_this.options.enableTrash) {
                ...
                jQuerycurrent_container = jQuery("." + current_container_class);
                jQueryprevious_container = jQuery("." + previous_container_class);
                jQueryselected = jQuery(selected.helper);
                jQuerycurrent_container.trigger("ss-trashed", jQueryselected);
                jQueryselected.remove();
                jQueryoriginal_container.trigger("ss-rearrange").removeClass(original_container_class);
                jQuerycurrent_container.trigger("ss-rearrange").removeClass(current_container_class);
                return jQueryprevious_container.trigger("ss-arrange").removeClass(previous_container_class);
              }
            }
          });
        }
      ...
      Plugin.prototype.setTargetPosition = function() {
        var jQueryselected, jQuerystart_container, jQuerytarget, attributes, child_positions, cutoff_end, cutoff_start, distance, dragged_class, options, parsed_children, placeholder_class, position_i, previous_container_class, selected_x, selected_y, shortest_distance, target_position, total_positions, x_dist, y_dist, _i;
        options = this.options;
        if (!options.enableTrash) {
          dragged_class = options.draggedClass;
          jQueryselected = jQuery("." + dragged_class);
          jQuerystart_container = jQueryselected.parent();
          parsed_children = this.parsedChildren;
          child_positions = this.getPositions(false);
          total_positions = child_positions.length;
          selected_x = jQueryselected.offset().left - jQuerystart_container.offset().left + (this.globals.col_width / 2);
          selected_y = jQueryselected.offset().top - jQuerystart_container.offset().top + (jQueryselected.height() / 2);
          ...
          if (total_positions > 1) {
            ...
            if (target_position === parsed_children.length) {
              jQuerytarget = parsed_children[target_position - 1].el;
              jQueryselected.insertAfter(jQuerytarget);
            } else {
              jQuerytarget = parsed_children[target_position].el;
              jQueryselected.insertBefore(jQuerytarget);
            }
          } else {
            if (total_positions === 1) {
              attributes = child_positions[0];
              if (attributes.left < selected_x) {
                this.jQuerycontainer.append(jQueryselected);
              } else {
                this.jQuerycontainer.prepend(jQueryselected);
              }
            } else {
              this.jQuerycontainer.append(jQueryselected);
            }
          ...
          this.arrange(true);
          if (jQuerystart_container[0] !== jQueryselected.parent()[0]) {
            previous_container_class = options.previousContainerClass;
            return jQuery("." + previous_container_class).trigger("ss-rearrange");
          }
        } else {
          placeholder_class = this.options.placeholderClass;
          return jQuery("." + placeholder_class).remove();
        }
      };
      ...
      Plugin.prototype.enableResize = function() {
        ...
        return jQuery(window).on(binding, function() {
          if (!resizing) {
            resizing = true;
            setTimeout((function() {
              return _this.render();
      ...
      Plugin.prototype.highestCol = function(array) {
        return jQuery.inArray(Math.max.apply(window, array), array);
      };

      Plugin.prototype.destroy = function() {
        var jQueryactive_children, jQuerycontainer, active_class;
        jQuerycontainer = this.jQuerycontainer;
        jQuerycontainer.off("ss-arrange");
        jQuerycontainer.off("ss-rearrange");
        jQuerycontainer.off("ss-setTargetPosition");
        jQuerycontainer.off("ss-destroy");
        active_class = this.options.activeClass;
        jQueryactive_children = jQuerycontainer.find("." + active_class);
        if (this.options.enableDrag) {
          jQueryactive_children.draggable('destroy');
        }
        if (this.options.enableCrossDrop) {
          jQuerycontainer.droppable('destroy');
        }
        jQueryactive_children.removeClass(active_class);
        return jQuerycontainer.removeClass(this.identifier);
      };

      return Plugin;

    })();
    return jQuery.fn[pluginName] = function(options) {
      return this.each(function() {
        var bound_indentifier, old_class, _ref;
        old_class = (_ref = jQuery(this).attr("class").match(/shapeshifted_container_\w+/)) != null ? _ref[0] : void 0;
        if (old_class) {
          bound_indentifier = "resize." + old_class;
          jQuery(window).off(bound_indentifier);
          jQuery(this).removeClass(old_class);
        }
        return jQuery.data(this, "plugin_" + pluginName, new Plugin(this, options));
      });
    };
